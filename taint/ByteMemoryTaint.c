 ...
 * following:
 * 
 * PM: Top most level of the hash map stored within the ByteMemTaint. Points to a SM
...
void chkByteMemDefaultVals(void) {
    ...
    for (i = 0; i < LVL_SIZE; i++) {
        ...
        if (i < LVL_SIZE && DQM->label[i] != 0) {
            printf("Bad DQM at %p, val: %llx\n", DQM, (unsigned long long)DQM->label[i]);
        }
    }
}
...
static inline void store8(ByteMemTaint *state, ADDRINT addr, const uint64_t *label) {
...
static inline void storeSlow(ByteMemTaint *state, ADDRINT addr, const uint64_t *labels, uint32_t sizeInBytes) {
    uint32_t i;
...
static inline void store16(ByteMemTaint *state, ADDRINT addr, const uint64_t *labels) {
...
static inline void store32(ByteMemTaint *state, ADDRINT addr, const uint64_t *labels) {
...
static inline void store64(ByteMemTaint *state, ADDRINT addr, const uint64_t *labels) {
...
void setAllByteMemTaint(ByteMemTaint *state, ADDRINT addr, uint32_t sizeInBytes, uint64_t label) {
    int i;
    for(i = 0; i < sizeInBytes; i++) {
...
void setByteMemTaint(ByteMemTaint *state, ADDRINT addr, uint32_t sizeInBytes, const uint64_t *labels) {
    if (sizeInBytes <= 8) {
        if (sizeInBytes == 4) {
...
static inline void loadSlow(ByteMemTaint *state, ADDRINT addr, uint32_t sizeInBytes, uint64_t *buffer) {
    uint32_t i;
    for (i = 0; i < sizeInBytes; i++) {
...
static inline void load16(ByteMemTaint *state, ADDRINT addr, uint64_t * buffer) {
    if (isNotAligned(addr, 2)) {
...
static inline void load32(ByteMemTaint *state, ADDRINT addr, uint64_t * buffer) {
    if (isNotAligned(addr, 4)) {
...
static inline void load64(ByteMemTaint *state, ADDRINT addr, uint64_t * buffer) {
    if (isNotAligned(addr, 8)) {
...
void getByteMemTaint(ByteMemTaint *state, ADDRINT addr, uint32_t sizeInBytes, uint64_t * buffer) {
    if (sizeInBytes <= 8) {
        if (sizeInBytes == 4) {
...
static inline void add8(ByteMemTaint *state, LabelStoreState *labelState, ADDRINT addr, uint64_t label) {
    QM *curQM = getQMForWriting(state, addr);
    uint32_t i = (addr & ADDR_MASK);
...
static inline void addSlow(ByteMemTaint *state, LabelStoreState *labelState, ADDRINT addr, uint64_t label, uint32_t sizeInBytes) {
    uint32_t i;
    for (i = 0; i < sizeInBytes; i++) {
...
static inline void add16(ByteMemTaint *state, LabelStoreState *labelState, ADDRINT addr, uint64_t label) {
    if (isNotAligned(addr, 2)) {
...
static inline void add32(ByteMemTaint *state, LabelStoreState *labelState, ADDRINT addr, uint64_t label) {
    if (isNotAligned(addr, 4)) {
...
static inline void add64(ByteMemTaint *state, LabelStoreState *labelState, ADDRINT addr, uint64_t label) {
    if (isNotAligned(addr, 8)) {
...
void addTaintToByteMem(ByteMemTaint *state, LabelStoreState *labelState, ADDRINT addr, uint32_t sizeInBytes, uint64_t label) {
    if (sizeInBytes <= 8) {
        if (sizeInBytes == 4) {
...
static inline uint64_t combine8(ByteMemTaint *state, LabelStoreState *labelState, ADDRINT addr, uint64_t curLabel) {
    QM *curQM = getQMForReading(state, addr);
...
static inline uint64_t combineSlow(ByteMemTaint *state, LabelStoreState *labelState, ADDRINT addr, uint32_t sizeInBytes, uint64_t curLabel) {
    uint32_t i;
    for (i = 0; i < sizeInBytes; i++) {
...
static inline uint64_t combine16(ByteMemTaint *state, LabelStoreState *labelState, ADDRINT addr, uint64_t curLabel) {
    if (isNotAligned(addr, 2)) {
...
static inline uint64_t combine32(ByteMemTaint *state, LabelStoreState *labelState, ADDRINT addr, uint64_t curLabel) {
    if (isNotAligned(addr, 4)) {
...
static inline uint64_t combine64(ByteMemTaint *state, LabelStoreState *labelState, ADDRINT addr, uint64_t curLabel) {
    if (isNotAligned(addr, 8)) {
...
uint64_t getCombinedByteMemTaint(ByteMemTaint *state, LabelStoreState *labelState, uint64_t addr, uint32_t sizeInBytes, uint64_t curLabel) {
    if (sizeInBytes <= 8) {
        if (sizeInBytes == 4) {
